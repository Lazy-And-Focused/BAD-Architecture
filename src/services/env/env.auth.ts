...
export const GROUPED_AUTH_PROPERTIES = Object.fromEntries(AUTH_TYPES.map(type => {
  const map = AUTH_DATA.map(data => [data, `${type}_${data}`] as [AuthData, AuthProperty]);
...
export const AUTH_PROPERTIES = Object.keys(GROUPED_AUTH_PROPERTIES).flatMap(key => {
  const properties = GROUPED_AUTH_PROPERTIES[key as Uppercase<AuthTypes>];
  return Object.keys(properties).map(property => properties[property as AuthData]);
});