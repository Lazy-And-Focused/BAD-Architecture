...
export const formatProperties = <
  ...
  Value = string,
  PropertyTransformerReturn extends string = Properties[number],
  RecordObject extends Record<PropertyTransformerReturn, Value> = Record<
    PropertyTransformerReturn,
    Value
  >,
>({
...
const transformAuthTypes = (data: AuthData) => {
  ...
  return formatProperties({
    properties,
    propertyTransformer: (property) =>
      `${property.toUpperCase()}_${data}` as `${Uppercase<typeof property>}_${typeof data}`,
    valueTransformer: defaultValueTransformer,
    errorTransformer: ({ property }) =>
      new Error(`Auth data ${data} for type ${property} is not defined in env`),
  });
};
